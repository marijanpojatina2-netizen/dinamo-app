...
const Step = { Login: 0, Choose: 1, Extras: 2, Review: 3, Payment: 4 };
// =============================================================
// VELIČINE PO TIPU
// =============================================================
const SHIRT_SIZES = ["110cm", "122cm", "134cm", "146cm", "158cm", "S", "M", "L", "XL", "2XL", "3XL"];
const HOODIE_SIZES = ["110cm", "122cm", "134cm", "146cm", "158cm", "S", "M", "L", "XL", "2XL", "3XL"];
const JERSEY_SIZES = ["110cm", "122cm", "134cm", "146cm", "158cm", "S", "M", "L", "XL", "2XL", "3XL", "4XL"];
// =============================================================
// UTIL FUNKCIJE
// =============================================================
...
function sanitizeHubText(input) {
    let s = input
...
function effectiveImage(p, overrideA, overrideB) {
    if (p.id === "A" && overrideA)
        return overrideA;
    if (p.id === "B" && overrideB)
        return overrideB;
...
const EXTRAS = CONFIG.products.extras;
const extrasById = Object.fromEntries(EXTRAS.map((e) => [e.id, e]));
const extrasTotal = (selected) => Array.from(selected).reduce((s, id) => s + (extrasById[id]?.price || 0), 0);
// =============================================================
// KOMPONENTA
// =============================================================
export default function App() {
    ...
    const [pkgSizes, setPkgSizes] = useState({
        jersey: "",
        shirt: "",
        hoodie: "",
    });
    const [customImageA] = useState(null);
    const [customImageB] = useState(null);
    const [resolvedImageA, setResolvedImageA] = useState(null);
    const [resolvedImageB, setResolvedImageB] = useState(null);
    // DODATNI ARTIKLI
    const [selectedExtras, setSelectedExtras] = useState(new Set());
    const [extraSizes, setExtraSizes] = useState({}); // id -> size
    // MAIL (bez UI-a)
    ...
    async function waitForRefs(getOk, timeoutMs = 1500, intervalMs = 50) {
        ...
        return new Promise((resolve, reject) => {
            const tick = () => {
                ...
                if (Date.now() - start > timeoutMs)
                    return reject(new Error("Canvas/container refs not mounted yet"));
    ...
    const handleBarcodeRef = useCallback((node) => {
    ...
    // Fokus zaštita
    useEffect(() => {
        if (step !== Step.Login || !activeField)
            return;
        const map = { first: firstRef, last: lastRef, coach: coachRef };
    ...
    useEffect(() => {
        ...
        if (!customImageA)
            chain(["/Paket1.jpg", "/paket1.jpg"], setResolvedImageA);
        if (!customImageB)
            chain(["/Paket2.jpg", "/paket2.jpg"], setResolvedImageB);
    }, [step, customImageA, customImageB]);
    const orderId = useMemo(() => `${CONFIG.paymentReferencePrefix}${new Date().toISOString().slice(0, 10)}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`, []);
    ...
    const canConfirmPackage = Boolean(selectedPackage) && pkgSizes.jersey !== "" && pkgSizes.shirt !== "" && pkgSizes.hoodie !== "";
    // svi odabrani extras koji imaju sizes moraju imati veličinu
    const allExtrasSizedOk = useMemo(() => {
        for (const id of selectedExtras) {
            const ex = extrasById[id];
            if (ex?.sizes && !(extraSizes[id] && extraSizes[id] !== ""))
                return false;
        }
        return true;
    }, [selectedExtras, extraSizes]);
    const hub2dPayload = useMemo(() => {
        if (!selectedPackage || !referenceNumber)
            return "";
    ...
    async function sendOrderNotification() {
        ...
        try {
            ...
            if (scriptUrl) {
                const extrasJson = JSON.stringify(Array.from(selectedExtras).reduce((acc, id) => {
                    const ex = extrasById[id];
    ...
    useEffect(() => {
        try {
            ...
            if (!p.includes("HRVHUB30"))
                throw new Error("HUB payload missing header");
    ...
    // UI helpers
    const PrimaryButton = ({ children, ...props }) => (<button type="button" {...props} className="px-5 py-3 rounded-2xl shadow-md font-semibold tracking-wide transition-transform active:scale-95 border border-black/10" style={{ background: CONFIG.theme.primary, color: "white" }}>
      {children}
    </button>);
    const OutlineButton = ({ children, ...props }) => (<button type="button" {...props} className="px-5 py-3 rounded-2xl border border-black/10 hover:border-black/30 transition-colors" style={{ color: CONFIG.theme.dark }}>
      {children}
    </button>);
    const Field = ({ id, label, children }) => (<div className="mb-4">
      <label htmlFor={id} className="block mb-2 text-sm font-medium text-black/80">
        {label}
      </label>
      {children}
    </div>);
    const Card = ({ children }) => (<div className="rounded-2xl border border-black/10 shadow-sm overflow-hidden bg-white">{children}</div>);
    const PackageCard = ({ pkg }) => (<Card>
      <div className="grid grid-cols-1">
        <div className="relative" style={{ aspectRatio: "16 / 9" }}>
          <img src={pkg.image} alt={pkg.name} className="absolute inset-0 w-full h-full object-contain bg-white" onError={(e) => {
        ...
        <div className="p-4 space-y-3">
          ...
          <ul className="text-sm text-black/70 list-disc pl-5">
            {pkg.includes.map((i) => (<li key={i}>
                {i}
                {i.toLowerCase().includes("double face dres") && (<div className="text-xs text-red-600 mt-1">Napomena: Broj dresa se dogovara s trenerom!</div>)}
              </li>))}
          </ul>
          <div className="pt-2">
        ...
        }} className={`w-full py-2.5 rounded-xl font-medium transition-all border ${selectedPackageId === pkg.id ? "bg-black text-white border-black" : "bg-white hover:bg-black/5 border-black/10"}`}>
    ...
    const ExtraCard = ({ extra, checked, onToggle, sizeValue, onSizeChange, }) => (<div className={`rounded-2xl border ${checked ? "border-black" : "border-black/10"} overflow-hidden bg-white shadow-sm transition-all`}>
      <div className="relative" style={{ aspectRatio: "1 / 1" }}>
        <img src={extra.image} alt={extra.label} className="absolute inset-0 w-full h-full object-cover" onError={(e) => {
      ...
      <div className="p-4 space-y-3">
        ...
        }} className={`w-full py-2.5 rounded-xl font-medium transition-all border ${checked ? "bg-black text-white border-black" : "bg-white hover:bg-black/5 border-black/10"}`}>
          {checked ? "Ukloni" : "Dodaj"}
        </button>
        {checked && Array.isArray(extra.sizes) && (<Field id={`extra-size-${extra.id}`} label="Veličina">
            <select value={sizeValue} onChange={(e) => onSizeChange(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10 bg-white">
              <option value="" disabled>Odaberi veličinu</option>
              {(extra.sizes ?? []).map((s) => (<option key={s} value={s}>{s}</option>))}
            </select>
          </Field>)}
      </div>
    </div>);
    return (<div className="min-h-screen bg-white text-black selection:bg-black/10">
      {/* Header */}
      <header className="sticky top-0 z-20 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white border-b border-black/10">
        <div className="w-full px-0 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3 pl-4">
            <div className="w-10 h-10 rounded-xl border border-black/10 grid place-items-center font-black" style={{ color: "white", background: CONFIG.theme.primary }}>
              D
            </div>
      ...
      <main className="w-full px-0 py-8 pb-[max(7rem,env(safe-area-inset-bottom))]">
        <AnimatePresence mode="wait">
          {/* PRIJAVA */}
          {step === Step.Login && (<motion.section key="login" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: "easeOut" }}>
                ...
                if (canContinueFromLogin)
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <Field id="firstName" label="Ime djeteta">
                        <input id="firstName" autoComplete="given-name" ref={firstRef} value={firstName} onFocus={() => setActiveField("first")} onChange={(e) => { setFirstName(e.target.value); setActiveField("first"); }} placeholder="npr. Luka" className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10"/>
                      </Field>
                      <Field id="lastName" label="Prezime djeteta">
                        <input id="lastName" autoComplete="family-name" ref={lastRef} value={lastName} onFocus={() => setActiveField("last")} onChange={(e) => { setLastName(e.target.value); setActiveField("last"); }} placeholder="npr. Horvat" className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10"/>
                      </Field>
                      <Field id="coach" label="Ime trenera">
                        <input id="coach" autoComplete="off" ref={coachRef} value={coach} onFocus={() => setActiveField("coach")} onChange={(e) => { setCoach(e.target.value); setActiveField("coach"); }} placeholder="npr. Marko" className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10"/>
                      </Field>
                    </div>
                    <div className="flex items-center justify-between mt-6">
                      <div className="text-sm text-black/60">Podaci se koriste samo za identifikaciju narudžbe.</div>
                      <PrimaryButton onClick={() => canContinueFromLogin && setStep(Step.Choose)} disabled={!canContinueFromLogin} title={!canContinueFromLogin ? "Ispuni sva polja" : "Nastavi"} style={{ opacity: canContinueFromLogin ? 1 : 0.6, cursor: canContinueFromLogin ? "pointer" : "not-allowed" }}>
                        Nastavi
                      </PrimaryButton>
                    </div>
                  </form>
          ...
          {/* ODABIR PAKETA */}
          {step === Step.Choose && (<motion.section key="choose" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: "easeOut" }}>
              <div className="mb-6 px-4">
              ...
              <div className="space-y-6 px-4">
                ...
                .map((p) => (<PackageCard key={p.id} pkg={p}/>))}
              </div>
                  ...
                  <div className="p-5">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                      <Field label="Veličina dresa" id="jersey">
                        <select value={pkgSizes.jersey} onChange={(e) => setPkgSizes((prev) => ({ ...prev, jersey: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10 bg-white">
                          <option value="" disabled>Odaberi veličinu</option>
                          {JERSEY_SIZES.map((s) => (<option key={s} value={s}>{s}</option>))}
                        </select>
                      </Field>
                      <Field label="Veličina majice" id="shirt">
                        <select value={pkgSizes.shirt} onChange={(e) => setPkgSizes((prev) => ({ ...prev, shirt: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10 bg-white">
                          <option value="" disabled>Odaberi veličinu</option>
                          {SHIRT_SIZES.map((s) => (<option key={s} value={s}>{s}</option>))}
                        </select>
                      </Field>
                      <Field label="Veličina hoodice" id="hoodie">
                        <select value={pkgSizes.hoodie} onChange={(e) => setPkgSizes((prev) => ({ ...prev, hoodie: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-black/10 focus:outline-none focus:ring-2 focus:ring-black/10 bg-white">
                          <option value="" disabled>Odaberi veličinu</option>
                    ...
                    <div className="flex items-center justify-between mt-6">
                      ...
                      <PrimaryButton onClick={() => canConfirmPackage && setStep(Step.Extras)} disabled={!canConfirmPackage} title={!canConfirmPackage ? "Odaberi paket i sve veličine" : "Potvrdi"} style={{ opacity: canConfirmPackage ? 1 : 0.6, cursor: canConfirmPackage ? "pointer" : "not-allowed" }}>
                        Potvrdi
                      </PrimaryButton>
                    </div>
                  </div>
          ...
          {/* DODATNI ARTIKLI */}
          {step === Step.Extras && (<motion.section key="extras" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: "easeOut" }}>
              <div className="mb-6 px-4">
                <h2 className="text-2xl font-bold tracking-tight" style={{ color: CONFIG.theme.dark }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-4">
                {EXTRAS.map((ex) => (<ExtraCard key={ex.id} extra={ex} checked={selectedExtras.has(ex.id)} sizeValue={extraSizes[ex.id] || ""} onSizeChange={(v) => setExtraSizes((prev) => ({ ...prev, [ex.id]: v }))} onToggle={() => {
                  ...
                  <div className="p-5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <PrimaryButton onClick={() => allExtrasSizedOk && setStep(Step.Review)} disabled={!allExtrasSizedOk} title={!allExtrasSizedOk ? "Odaberi veličine za odabrane artikle" : "Potvrdi"} style={{ opacity: allExtrasSizedOk ? 1 : 0.6, cursor: allExtrasSizedOk ? "pointer" : "not-allowed" }}>
                          Potvrdi
                        </PrimaryButton>
                      </div>
                    </div>
          ...
          {/* REKAPITULACIJA */}
          {step === Step.Review && (<motion.section key="review" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: "easeOut" }}>
              <div className="mb-6 px-4">
                <h2 className="text-2xl font-bold tracking-tight" style={{ color: CONFIG.theme.dark }}>
                  Rekapitulacija
                </h2>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <ul className="text-sm">
                          {selectedPackage?.includes ? (selectedPackage.includes.map((i) => (<li key={i}>
                                • {i}
                                {i.toLowerCase().includes("double face dres") && (<span className="text-red-600"> — Napomena: Broj dresa se dogovara s trenerom!</span>)}
                              </li>))) : (<li>—</li>)}
                        </ul>
                      </div>
                      <div>
                      ...
                      {CONFIG.emailWebhook ? (<div className="text-xs text-black/70" aria-live="polite">
                          {emailStatus === "idle" && (<>Narudžba je obvezujuća. Naručitelj se obvezuje uplatiti u roku od 7 dana. Detalji narudžbe bit će poslani na <b>oprema@kkdinamo.hr</b>.</>)}
                          {emailStatus === "sending" && <>Šaljemo potvrdu narudžbe…</>}
                          {emailStatus === "sent" && <>Potvrda narudžbe poslana na e-mail.</>}
                          {emailStatus === "error" && (<span className="text-red-600">Slanje potvrde nije uspjelo. Molimo napravite screenshot ove stranice.</span>)}
                        </div>) : (<div className="text-xs text-black/70">Narudžba je obvezujuća. Naručitelj se obvezuje uplatiti u roku od 7 dana.</div>)}
                    </div>
                  </div>
                </Card>
              </div>
          ...
          {/* UPLATA */}
          {step === Step.Payment && (<motion.section key="payment" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: "easeOut" }}>
              <div className="mb-6 px-4">
                <h2 className="text-2xl font-bold tracking-tight" style={{ color: CONFIG.theme.dark }}>
                  Podaci za uplatu
                </h2>
                  ...
                  <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-3">
                        ...
                        <PrimaryButton onClick={() => window.print()}>Ispis / PDF</PrimaryButton>
                      </div>
                    </div>
